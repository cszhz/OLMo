...
def main(cfg: TrainConfig) -> None:
    ...
    # Set device (check environment variable for XLA/Neuron).
    is_xla = os.environ.get("USE_XLA", "0") == "1"

    if is_xla:
        log.info("USE_XLA=1, attempting to use XLA/Neuron device...")
        try:
            import torch_xla.core.xla_model as xm
            # For multi-process training, each process gets its own XLA device
            # xm.xla_device() will automatically select the correct device based on local rank
            device = xm.xla_device()
            device_type = xm.xla_device_hw(device)
            local_rank = get_local_rank()
            log.info(f"Using XLA device: {device} (type: {device_type}, local_rank: {local_rank})")
        except Exception as e:
            log.error(f"Failed to initialize XLA device: {e}")
    ...
    # Wrap data loader for XLA if needed
    if is_xla:
        log.info("Wrapping data loader for XLA...")
        import torch_xla.distributed.parallel_loader as pl
        original_train_loader = train_loader
        train_loader = pl.MpDeviceLoader(train_loader, device)
        # Preserve dataset attribute for Trainer
        train_loader.dataset = original_train_loader.dataset
    ...
    with Trainer(
        ...
        # Wrap train_step for XLA if needed
        if is_xla:
            import torch_xla.core.xla_model as xm

            world_size = get_world_size()
            log.info(f"Wrapping trainer with XLA synchronization (world_size={world_size})...")

            original_train_step = trainer.train_step
            original_optim_step = trainer.optim.step

            if world_size > 1:
                # Multi-process: replace optimizer.step with xm.optimizer_step
                log.info("Multi-process XLA training: using xm.optimizer_step for gradient synchronization")

                def xla_optimizer_step(closure=None):
                    # Use XLA's optimizer_step which handles gradient synchronization
                    xm.optimizer_step(trainer.optim, optimizer_args={'closure': closure})

                def xla_train_step_multi(batch, reduce_global_loss=True):
                    # Temporarily replace optimizer.step
                    trainer.optim.step = xla_optimizer_step
                    try:
                        metrics = original_train_step(batch, reduce_global_loss=reduce_global_loss)
                    finally:
                        # Restore original step method
                        trainer.optim.step = original_optim_step
                    xm.mark_step()  # Execute all pending operations
                    return metrics

                trainer.train_step = xla_train_step_multi
            else:
                # Single process: just add mark_step
                def xla_train_step(batch, reduce_global_loss=True):
                    metrics = original_train_step(batch, reduce_global_loss=reduce_global_loss)
                    xm.mark_step()  # XLA synchronization point
                    return metrics

                trainer.train_step = xla_train_step
...
if __name__ == "__main__":
    ...
    # Initialize distributed backend (use gloo/nccl, not XLA backend for compatibility)
    ...
    # Get world size to determine if multi-process training
    world_size = dist.get_world_size() if dist.is_initialized() else 1

    # Update config based on available hardware
    if torch.backends.mps.is_available():
        log.info("Device is MPS. Updating config...")
        cfg.model.init_device = "mps"
        cfg.distributed_strategy = "single"  # type: ignore
    elif not torch.cuda.is_available():
        log.info(f"Device is CPU or XLA. Updating config (world_size={world_size})...")
        cfg.model.init_device = "cpu"  # Initialize on CPU, then move to device
        # Only use "single" strategy for single process
        if world_size == 1:
            cfg.distributed_strategy = "single"  # type: ignore
        # For multi-process, keep the config's distributed_strategy (fsdp/ddp)
